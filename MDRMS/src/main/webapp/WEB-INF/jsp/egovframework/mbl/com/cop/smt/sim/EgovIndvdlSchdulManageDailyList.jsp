   			...
   			$.getJSON("${pageContext.request.contextPath}/cop/smt/sim/EgovIndvdlSchdulManageDailyListActor.mdo", $('#searchVO').serialize().replace(/%/g,'%25'), function(json){
				...
				if(json.resultList.length == 0) {
					...
					for ( var i = 0; i < json.resultList.length; i++) {
						...
						var result =  json.resultList[i];
						var regularHour = result.worktimeRegular;
						var extensionHour = result.worktimeExtension;
						var overnightHour = result.worktimeOvernight;
						
						//var beginDe = result.schdulBgnde.substring(8, 10) + '시 ' + result.schdulBgnde.substring(10, 12) + '분';
						//var endDe = result.schdulEndde.substring(8, 10) + '시 ' + result.schdulEndde.substring(10, 12) + '분';
						
						html += '<li>';
						html += '	<a href="javascript:fn_show_view(\'' + result.diaryId + '\');" >';
						html += '		<h3>' + result.schdulChargerName + '</h3>';
						html += '		<p class="uss-darkgray">' +'정상:'+ regularHour + ',잔업:' + extensionHour +',철야:'+overnightHour+'</p>';
						html += '	</a>';
						html += '</li>';	
...
<%-- 					<li><a href="${pageContext.request.contextPath}/cop/smt/sim/EgovIndvdlSchdulManageWeekList.mdo" id="week" name="week" data-ajax="false">주간별 일정보기</a></li> --%>
					<li><a href="${pageContext.request.contextPath}/cop/smt/sim/EgovIndvdlSchdulManageDailyList.mdo" class="ui-btn-active" id="date" name="date" data-ajax="false">일별 일보보기</a></li>
				</ul>
			</div>
		</div>			
		<!-- header end -->
